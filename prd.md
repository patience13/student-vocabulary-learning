...
【其他说明】
在运行和修改该项目代码时，请严格注意以下约束：

图片生成必须通过“提示词（prompt）→ 调用图片生成 API”的方式完成，不允许使用本地图片或占位图代替最终功能。

API Key 仅允许本地存储和使用：

不得将 API Key 硬编码在代码中
不得提交到仓库
可通过本地配置文件或环境变量读取（如 .env、本地 config）
✅ 允许在功能的交互界面中向用户询问其 API Key，并在用户输入后，仅在当前本地环境中使用该 Key 进行 API 调用，不得持久化上传或外泄
项目技术栈限定为纯前端实现：

仅使用 HTML、CSS、原生 JavaScript
不使用任何前端或后端框架（如 React、Vue、Node.js 服务等）
在不确定实现方式前，请先说明你的实现方案，再开始写代码。